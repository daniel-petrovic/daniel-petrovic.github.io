...
The implementation in my [linux-system-playground](https://codeberg.org/daniel-petrovic/linux-system-playground) is a small C99-style, typed dynamic array. It is not an STL implementation--and it does not try to be one. Its purpose is to expose the machinery that makes a vector useful: contiguous storage, a logical size, allocated capacity, and controlled reallocation.
